...
import ru.iesorokin.ordermanager.orchestrator.core.enums.bpmn.BusinessProcessEvent
import ru.iesorokin.ordermanager.orchestrator.core.enums.bpmn.Process
import ru.iesorokin.ordermanager.orchestrator.core.exception.StartProcessException
...
class CamundaService(private val runtimeService: RuntimeService) {
    ...
    fun getRefundTpNetProcessByUniqueVariableForProcess(variableName: String,
        ...
        try {
            ...
            log.error { "Process with variable $variableName == $value is not single: ${e.message}" }
            throw e
        } catch (e: KotlinNullPointerException) {
            log.error {
                "There is now active instance with variable $variableName == $value"
            }
            throw e
        } catch (e: Throwable) {
            log.error(e) {
    ...
    fun getSubscriptionByProcessInstanceIdAndEventName(processInstanceId: String,
            ...
            try {
                ...
                log.error {
                    "There is now active instance with processInstanceId = $processInstanceId and eventName = $eventName"
                }
                throw e
            } catch (e: Throwable) {
                log.error(e) {
